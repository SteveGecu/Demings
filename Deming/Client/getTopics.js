...
async function getRovrLogMessage(dsn) {
    ...
    const dsnMessage = messages.find(m => m.data.dsn == dsn)
    return dsnMessage
}
...
async function getRovrTelemetryReportMessage(dsn) {
    ...
    const dsnMessage = messages.find(m => m.data.provisioning.dsn == dsn)
    return dsnMessage
}
...
async function getRovrUploadReportMessage(dsn) {
    ...
    const dsnMessage = messages.find(m => m.Key.slice(20, 32) == dsn)
    return dsnMessage
}
